...
install: schemas
	rm -rf $(INSTALL_DIR)
	mkdir -p $(INSTALL_DIR)
	cp extension.js prefs.js metadata.json stylesheet.css LICENSE $(INSTALL_DIR)/
...
check:
	...
	@glib-compile-schemas --strict --dry-run schemas/ && echo "ok   schemas"
	@python3 -c "import json;json.load(open('metadata.json'))" && echo "ok   metadata.json"
